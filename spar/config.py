...
def _validate_side_config(side_name: str, config: dict) -> None:
    ...
    allowed_keys = {"adapter", "command", "model"}
    for key in config.keys():
        if key not in allowed_keys:
...
def _dict_to_config(config_dict: dict) -> Config:
    ...
    allowed_top_level = {"sides", "debate"}
    ...
    for side_name, side_config in sides_dict.items():
        ...
        sides[side_name] = SideConfig(adapter=adapter, command=command, model=model)
    ...
    return Config(sides=sides, debate=debate)
...
def load_config(project_dir: Path, global_path: Optional[Path] = None) -> Config:
    ...
    defaults_dict = {
        "sides": {
            name: {"adapter": side.adapter, "command": side.command, "model": side.model}
